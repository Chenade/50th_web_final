...
abstract class BaseTest extends TestCase
{
     ...
     */
    protected function login($username = 'sysop')
    {
        return $this->http->post($this->api_prefix . '/login', [
     ...
     */
    protected function getLoginToken($user = 'sysop')
    {
        $res = $this->login($user);

        if ($res->getStatusCode() === 200 && isset($res->getJson()['data'])) {
            return $res->getJson()['data'];
        }
